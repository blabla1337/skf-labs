...
for i in $(ls -d */); 
    do 
    dir=${i%%/};
    echo "docker login; cd" $dir"; rm -rf node_modules; docker buildx build --platform linux/amd64,linux/arm/v7 -t blabla1337/owasp-skf-lab:js-"$dir " --push .; cd .." | tr "[:upper:]" "[:lower:]" 
done
