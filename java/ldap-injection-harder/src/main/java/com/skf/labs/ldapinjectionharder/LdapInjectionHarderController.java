...
@Controller
public class LdapInjectionHarderController {
    


    @PostMapping("/login")
	public String login(@RequestParam(name="username", required=true) String username,@RequestParam(name="secret_answer", required=true) String secret_answer,Model model) throws javax.naming.NamingException{
        if(secret_answer.length() < 2){
            model.addAttribute("content","The secret answer is at least 2 character long.");
            return "index";
