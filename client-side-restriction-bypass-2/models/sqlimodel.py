...
class Classes:
    ...
    def getFood(self, userId):
        db = database_con()
        cur = db.execute('SELECT Food FROM prefs WHERE UserId=?',
        [userId])
        return cur.fetchall()
        
    def updateFood(self, food, userId):
        db = database_con()
        cur = db.execute('UPDATE prefs SET Food=? WHERE UserId=?',
        [food, userId])
        db.commit()
        return cur.fetchall()
